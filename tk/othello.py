...
depth = 4
moves = 0
#Sunzhg, level == 1 is random,level == 2 is greedy, level == 3 is minmax(writen by others now)
level = 1
	...
	def update(self):
		...
		if not self.won:
			...
			if self.player==1:
				startTime = time()
				self.oldarray = self.array
				#Sunzhg, Result[v,newboard,newposition] is a list of result of ai function
				Result = []
				if level==1:
					Result = self.random()
				if level==2:
					Result = self.greedy()
				if level==3:
					Result = self.alphaBeta(self.array,depth,-float("inf"),float("inf"),1)
				self.array = Result[1]
			...
			if player_score<computer_score:
				screen.create_text(250,480,anchor="c",font=("Consolas",15), text="The game is done! Computer win!!")
			else:
				screen.create_text(250,480,anchor="c",font=("Consolas",15), text="The game is done! Player win!!")
	#Moves to position
		...
		for x in range(8):
			for y in range(8):
					if valid(self.array,self.player,x,y):
							choices.append([x,y])
		if len(choices)==0:
			return ([finalHeuristic(self.array,1),self.array])
		index = random.randint(0,len(choices)-1)
		position = choices[index]
		return [0,move(self.array,position[0],position[1]),position]
