...
class Gui:
    def __init__(self):
        ...
        self.clear_img = pygame.image.load(os.path.join("res",
                                                        "nada.bmp")).convert()
        self.remain_img = pygame.image.load(os.path.join("res",
                                                        "remain.jpg")).convert()
    ...
    def get_chosen_level(self):
        ...
        while True:
            ...
            title = title_fnt.render("Othello", True, self.WHITE)
            title_pos = title.get_rect(
                centerx=self.screen.get_width() / 2, centery=40)
            one_txt = self.font.render("Level 1", True, self.WHITE)
            one_pos = one_txt.get_rect(
                centerx=self.screen.get_width() / 2, centery=90)
            ...
            three_pos = three_txt.get_rect(
                centerx=self.screen.get_width() / 2, centery=270)
            four_txt = self.font.render("Level 4", True, self.WHITE)
            four_pos = four_txt.get_rect(
                centerx=self.screen.get_width() / 2, centery=360)
            ...
            self.screen.blit(four_txt, four_pos)
            ...
            for event in pygame.event.get():
                if event.type == QUIT:
                    ...
                    if one_pos.collidepoint(mouse_x, mouse_y):
                        ...
                        return 3                    
                    elif four_pos.collidepoint(mouse_x, mouse_y):
                        return 4
    ...
    def update(self, board, blacks, whites, current_player_color,valid_places):
        """Updates screen
        """
        img = self.remain_img
        if current_player_color==WHITE:
            for i in self.wremain:
                self.clear_square(i)
            if len(valid_places) != 0:
                self.bremain = valid_places
        else:
            for i in self.bremain:
                self.clear_square(i)         
            if len(valid_places) != 0:
                self.wremain = valid_places
        for i in valid_places:
            x = i[1] * self.SQUARE_SIZE + self.BOARD[0] + 16
            y = i[0] * self.SQUARE_SIZE + self.BOARD[1] + 14
            self.screen.blit(img, (x, y), img.get_rect())
